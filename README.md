...
*SweetClocker keeps your device running cool and smooth by capping CPU frequencies at their most efficient sweet spots, or allowing full custom cluster tuning up to hardware boost clocks.*
...
- **Custom Cluster Frequency Tuning**: Set custom min and max frequency limits per cluster (CPUs 0-1, 2-4, 5-6, 7) directly from the WebUI up to full hardware boost clocks (up to **3.21 GHz** on Prime, **3.01 GHz** on Big).
...
- **Material 3 Expressive WebUI**: Embedded dashboard with dynamic Monet color sync, interactive M3 bottom sheet dialogs, smooth spring fly-in animations, and glassmorphic aesthetics.
- **Namespace-Immune Enforcement**: Uses an isolated secondary `sysfs` mount (`/dev/sweetclocker/sysfs`) to prevent thermal daemons and PowerHAL from bypassing limits.
...
4. Open the **SweetClocker** WebUI card inside KernelSU / APatch / MMRL to monitor thermals or customize cluster frequencies.
